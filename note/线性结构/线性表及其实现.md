...
如下图，每个节点都有两个值，一个存储数据，另一个存储下一个节点的值。
...
​	每次查找都会比较一次并且判断是否下一个节点是否为空，将这个操作作为基本操作。
...
​	这里每次都需要从第一个节点遍历到最后，时间复杂的为 $O(n) $

​	如果同时存储最后一个节点的坐标，时间复杂度可以降为 $O(1) $
...
​	当前获取元素个数时，每次都需要从第一个节点遍历到最后，时间复杂的为 $O(n) $
